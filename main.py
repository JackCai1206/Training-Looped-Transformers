...
def evaluate(model, step, epoch, eval_loader, criterion, sim, args):
    ...
    if args.wandb:
        wandb.log({ 'val_acc': num_correct / num_el, 'epoch': epoch}, step=step, commit=False)
...
def main(args, checkpoint):
    ...
    try:
        ...
        for epoch in range(epoch_resume, args.epochs+1):
            ...
            while not stop_eval:
                try:
                    val_loss, val_acc = evaluate(model, step, epoch, eval_loader, criterion, test_sim, args)
                    stop_eval = True
                except RuntimeError as e:
                    if 'out of memory' in str(e):
                        args.eval_batch_size = int(args.eval_batch_size / 1.2)
                        args.num_eval_batches = args.num_valid // args.eval_batch_size
                        print('| WARNING: ran out of memory, reducing eval batch size to {:5d}'.format(args.eval_batch_size))
                        # if args.eval_batch_size < 100:
                        #     stop_eval = True
                        eval_loader.dataset.clear_cache()
                        eval_loader = DataLoader(eval_loader.dataset, batch_size=args.eval_batch_size, shuffle=False, drop_last=True)
                        # clear gpu data
                        # model.zero_grad(set_to_none=True)
                    else:
                        raise e
