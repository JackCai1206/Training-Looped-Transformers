...
class SubleqSim():
    ...
    def create_state(self):
        ...
        assert torch.sum(self.X[self.PC_slice]) == 0
        self.X[self.PC_slice] = PC
        assert torch.sum(self.X[self.pos_enc_slice]) == 0
        self.X[self.pos_enc_slice] = pos_enc
        assert torch.sum(self.X[self.scratch_ind_slice]) == 0
        self.X[self.scratch_ind_slice] = scratch_ind
        assert torch.sum(self.X[self.M_slice]) == 0
        self.X[self.M_slice] = M
        assert torch.sum(self.X[self.C_slice]) == 0
