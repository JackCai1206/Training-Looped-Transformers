...
def to_signed_binary(x: torch.Tensor):
    return (x * 2 - 1) * 100

# Convert -1000/1000 binary matrix into 0/1 binary matrix
def from_signed_binary(x: torch.Tensor):
    return (x // 100 + 1) // 2
...
def to_binary_col(x: torch.Tensor, bits):
    ...
    return to_signed_binary((x.unsqueeze(1) & mask).T.ne(0).float())
...
class SubleqSim():
    def __init__(self, N, s, m, n):
        ...
        self.d = d = max(N, 3*log_n) + log_n + 1
        ...
        self.M_slice = slice(d-log_n-N-1,d-log_n-1), slice(s,s+m)
        # self.C_slice = slice(0,3*log_n), slice(s+m,n)
        self.C_slice = slice(d-4*log_n-1,d-log_n-1), slice(s+m,n)
        self.create_state()
    ...
    def create_state(self):
        ...
        scratch_ind = to_signed_binary(torch.cat([torch.ones(1, s), torch.zeros(1, n-s)], dim=1))
        M = to_binary_col(torch.randint(0, 2**N, (m,)), N)
        C = torch.cat([
        ...
        # assert torch.sum(self.X[self.PC_slice]) <= 10
        self.X[self.PC_slice] = PC
        # assert torch.sum(self.X[self.pos_enc_slice]) <= 10
        self.X[self.pos_enc_slice] = pos_enc
        # assert torch.sum(self.X[self.scratch_ind_slice]) <= 10
        self.X[self.scratch_ind_slice] = scratch_ind
        # assert torch.sum(self.X[self.M_slice]) <= 1
        self.X[self.M_slice] = M
        # assert torch.sum(self.X[self.C_slice]) <= 1
